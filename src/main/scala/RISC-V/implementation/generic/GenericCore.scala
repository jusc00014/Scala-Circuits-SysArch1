...
//Erstelle Schnittstellen für reset, instruction, data und RISC-V-Formalschnittstelle
  val io_reset = IO(new ResetInterface)
  val io_instr = IO(new InstructionInterface)
  val io_data = IO(new DataInterface)
  val io_rvfi = IO(new RVFIInterface) //Enthält Infos über den Prozessor

//Initialisiere execution units
  val executionUnits = genExecutionUnits.map( gen => Module(gen())) 

//Initialisiere und verbinde das ProgramCounter modul
...
//Initialisiere und verbinde das RegisterFile modul
...
//Initialisiere data Schnittstellen 
...
//Erstelle eine Verbindung für misa, das angibt, welche Erweiterungen unterstützt werden
  val misa = Wire(UInt(32.W))
  misa := executionUnits.map(_.io.misa).reduce(_ | _)

//Erstellt eine Verbindung für stall reason, die angibt, ob mehrere Zyklen für eine Instruktion benötigt werden
  val stall = Wire(STALL_REASON())
  when (io_instr.instr_gnt) {
    stall := STALL_REASON.NO_STALL //Kein weiterer Zyklus benötigt
  } .otherwise {
    stall := STALL_REASON.INSTR_REQ
  }
  val was_stalled = RegNext(stall)

//Welche instruktion kommt als nächstes?
...
//Handle data grant, data read Signale, instruction request und address
...
//Verbinde ExecutionUnits mit dem Kern
...
//RVFI Interface Signale
  ...
  //Memory address und data Signale
  val mem_addr = RegInit(0.U(32.W))
  when (~io_reset.rst_n) {    //Reset der mem_addr?
    mem_addr := 0.U
  } .elsewhen (io_data.data_req) {    //Werden Daten angefordert?
  ...
  when (~io_reset.rst_n) {    //Reset der rmask und rdata?
    mem_rmask := 0.U
    mem_rdata := 0.U
    mem_wmask := 0.U
    mem_wdata := 0.U
  } .elsewhen (io_data.data_req & io_data.data_we) {  //Daten lesen und schreiben?
    mem_rmask := 0.U
  ...
  } .elsewhen(io_data.data_req) {   //Daten lesen?
